const tags = ['cheapest', 'best-price', 'lowest-clean-fee'];
const emojis = ['😊', '🌟', '💰', '🏡', '🔥', '🌈', '🌍', '🚀', '⭐', '🎉'];

// Export với alias name "misc"
export const misc = { tags, emojis };
console.log(misc);

// Hàm tính phần trăm khuyến mãi
export function promotionPercentCalc(price, promotionPrice) {
  const priceNum = parseFloat(price.replace(/[$,]/g, '')); // Loại bỏ dấu '$' và dấu ','.
  const promotionPriceNum = parseFloat(promotionPrice.replace(/[$,]/g, '')); // Loại bỏ dấu '$' và dấu ','.
  const discountPercent = ((priceNum - promotionPriceNum) / priceNum) * 100;
  return Math.round(discountPercent * 100) / 100; // Làm tròn đến 2 chữ số thập phân.
}
